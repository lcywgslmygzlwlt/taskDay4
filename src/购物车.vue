...
export default {
  ...
  methods: {
    ...
    }, // sumCommodity(){ //   return this.list.filter(item => item.a == true).reduce((t,item)=>(t+=item),0) // }
  },
  computed: {
    isAll: {
      set(val) {
      ...
      get() {
        return this.list.length === 0
          ? false
          : this.list.every((ele) => ele.a == true) //  return this.list.every((ele)=>(ele.a == true))
      },
    },
    sumCommodity() {
      return this.list
    ...
    }, // add(index){ //   this.list[index].num++ // }, // sub(){ //   this.list[index].num-- // }
  },
}
</script>
<style>
